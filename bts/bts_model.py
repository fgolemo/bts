...
class local_planar_guidance(nn.Module):
    ...
    def forward(self, plane_eq, focal):
        plane_eq_expanded = torch.repeat_interleave(plane_eq, int(self.upratio),
                                                    2)
        ...
        u = self.u.repeat(
            plane_eq.size(0),
            plane_eq.size(2) * int(self.upratio), plane_eq.size(3)).to(device)
        u = (u - (self.upratio - 1) * 0.5) / self.upratio

        v = self.v.repeat(
            plane_eq.size(0), plane_eq.size(2),
            plane_eq.size(3) * int(self.upratio)).to(device)
...
class bts(nn.Module):
    ...
    def __init__(self, params, feat_out_channels, num_features=512):
        ...
        self.daspp_6 = atrous_conv(
            ...
            num_features // 4, 6)
        self.daspp_12 = atrous_conv(num_features + feat_out_channels[2],
                                    num_features // 4, 12)
        ...
        self.conv1 = torch.nn.Sequential(
            nn.Conv2d(
                num_features // 16 + 4, num_features // 16, 3, 1, 1,
                bias=False), nn.ELU())
